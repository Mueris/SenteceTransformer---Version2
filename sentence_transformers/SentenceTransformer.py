...
class SemanticSearchIndex:
    """Lightweight semantic search index for reuse across queries."""
...
class SentenceTransformer(nn.Sequential, FitMixin, PeftAdapterMixin):
    ...
    def clear_cache(self) -> None:
        """Clear the embedding cache and reset cache statistics.

        This method removes all cached embeddings and resets the hit/miss counters.
        Use this when you want to free up memory or start fresh with caching.
    ...
    def cache_stats(self) -> dict[str, int]:
        ...
        Returns a dictionary with the following keys:
            - ``entries``: Total number of cached embeddings
            - ``hits``: Number of cache hits (embeddings retrieved from cache)
            - ``misses``: Number of cache misses (embeddings computed)
        ...
        Example:
            ::
                ...
                # First call - all sentences are cache misses
                sentences = ["The weather is lovely today.", "It's so sunny outside!"]
                embeddings = model.encode(sentences, use_cache=True)
                print(model.cache_stats())
                # {'entries': 2, 'hits': 0, 'misses': 2}

                # Second call with some repeated sentences - some cache hits
                more_sentences = ["The weather is lovely today.", "New sentence here."]
                embeddings2 = model.encode(more_sentences, use_cache=True)
                print(model.cache_stats())
    ...
    def semantic_search(
        ...
        This helper method combines encoding and similarity search for convenience.
        It embeds the query and corpus, computes similarity scores, and returns the
        top-k most similar corpus entries.
    ...
    def semantic_search_batch(
        ...
        This helper method encodes all queries, computes similarity scores between
        each query and the corpus, and returns the top-k most similar corpus entries
        for each query.
        ...
        Example:
            ::
                ...
                results = model.semantic_search_batch(queries, corpus, top_k=2, use_cache=True)
    ...
    def build_search_index(
        ...
        """Build a reusable semantic search index.

        The index stores the corpus texts and their embeddings so they can be reused
        across multiple search requests without recomputing embeddings.
        ...
        Returns:
            SemanticSearchIndex: An index object with ``search`` and ``search_batch`` helpers.
        ...
        Example:
            ::
                ...
                model = SentenceTransformer("all-MiniLM-L6-v2")
                corpus = ["The weather is lovely today.", "Pizza is delicious."]

                index = model.build_search_index(corpus, use_cache=True)
                results = index.search("Tell me about pizza", top_k=1)
        """
        if corpus_embeddings is None:
            corpus_embeddings = self.encode(corpus, convert_to_tensor=True, use_cache=use_cache)
    ...
    def encode(
        ...
        **kwargs,
        ...
        Args:
            ...
            chunk_size (int, optional): Size of chunks for multi-process encoding. Only used with multiprocessing, i.e. when
                ``pool`` is not None or ``device`` is a list. If None, a sensible default is calculated. Defaults to None.
            use_cache (bool, optional): Whether to use the embedding cache. If True, the model will cache embeddings
                for identical inputs and reuse them on subsequent calls. This can significantly speed up encoding
                when there are many duplicate texts. The cache is stored in memory only. Defaults to False.
        ...
        cache_hits = [None] * len(sentences)
        sentences_to_encode = []
        sentence_indices = []
        if use_cache:
            for idx, sentence in enumerate(sentences):
                if sentence in self._embedding_cache:
                    cache_hits[idx] = self._embedding_cache[sentence]
        ...
        all_embeddings = []
        length_sorted_idx = np.argsort([-self._text_length(sen) for sen in sentences_to_encode])
        sentences_sorted = [sentences_to_encode[int(idx)] for idx in length_sorted_idx]

        for start_index in trange(0, len(sentences_to_encode), batch_size, desc="Batches", disable=not show_progress_bar):
        ...
        # Store computed embeddings in cache and reconstruct output with cached embeddings
        if use_cache:
            # Store new embeddings in cache
            for idx, embedding in zip(sentence_indices, all_embeddings):
                self._embedding_cache[sentences[idx]] = embedding

            # Reconstruct full output including cached embeddings
            reconstructed_embeddings = []
            for idx in range(len(sentences)):
                if cache_hits[idx] is not None:
                    reconstructed_embeddings.append(cache_hits[idx])
                else:
                    # Find the corresponding embedding in all_embeddings
                    embedding_idx = sentence_indices.index(idx)
                    reconstructed_embeddings.append(all_embeddings[embedding_idx])
            all_embeddings = reconstructed_embeddings
