...
For improved performance when encoding texts with many duplicates, you can enable the embedding cache. When `use_cache=True`, the model will cache embeddings for identical inputs and reuse them on subsequent calls, avoiding redundant computation.
...
# Example with duplicate texts
sentences = [
    "The weather is lovely today.",
    "It's so sunny outside!",
    "The weather is lovely today.",  # Duplicate
    "He drove to the stadium.",
    "It's so sunny outside!",  # Duplicate
]

# First call computes embeddings for unique texts
embeddings = model.encode(sentences, use_cache=True)

# Subsequent calls with the same texts will use cached embeddings
more_sentences = [
    "The weather is lovely today.",  # Already in cache
    "New sentence here.",  # Will be computed and cached
]
embeddings2 = model.encode(more_sentences, use_cache=True)
```

The cache is stored in memory only and persists for the lifetime of the model instance.
...
print(stats)
# {'entries': 10, 'hits': 25, 'misses': 15}
```

The `cache_stats()` method returns a dictionary with:
- `entries`: Total number of cached embeddings
- `hits`: Number of embeddings retrieved from cache
- `misses`: Number of embeddings computed (not in cache)
...
You can build a reusable semantic search index to avoid recomputing corpus embeddings across multiple searches.
...
# Single query search
results = index.search("Tell me about pizza", top_k=2)
for result in results:
    print(result["text"], result["score"])

# Batch query search
queries = ["Weather today", "Tell me about pizza"]
batch_results = index.search_batch(queries, top_k=2)
